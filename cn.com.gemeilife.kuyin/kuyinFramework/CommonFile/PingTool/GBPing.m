...
-(void)setTimeout:(NSTimeInterval)timeout {
    @synchronized(self) {
        if (self.isPinging) {
            if (self.debug) {
                DLog(@"GBPing: can't set timeout while pinger is running.");
            }
        }
        else {
            _timeout = timeout;
...
-(void)setTtl:(NSUInteger)ttl {
    @synchronized(self) {
        if (self.isPinging) {
            if (self.debug) {
                DLog(@"GBPing: can't set ttl while pinger is running.");
            }
        }
        else {
            _ttl = ttl;
...
-(void)setPayloadSize:(NSUInteger)payloadSize {
    @synchronized(self) {
        if (self.isPinging) {
            if (self.debug) {
                DLog(@"GBPing: can't set payload size while pinger is running.");
            }
        }
        else {
...
-(void)setPingPeriod:(NSTimeInterval)pingPeriod {
    @synchronized(self) {
        if (self.isPinging) {
            if (self.debug) {
                DLog(@"GBPing: can't set pingPeriod while pinger is running.");
            }
        }
        else {
...
-(void)setupWithBlock:(StartupCallback)callback {
    ...
    if (self.isReady) {
        if (self.debug) {
            DLog(@"GBPing: Can't setup, already setup.");
        }
    ...
    if (!self.host) {
        if (self.debug) {
            DLog(@"GBPing: set host before attempting to start.");
        }
...
-(void)sendPing {
    if (self.isPinging) {
        ...
        //failed to send
            ...
            //little log
            if (self.debug) {
                DLog(@"GBPing: failed to send packet with error code: %d", err);
            }
