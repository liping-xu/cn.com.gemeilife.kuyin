...
- (void)viewInit
{
    ...
    CGFloat navTabBarX = 0.0f;
//    CGFloat navTabBarY = [UIDevice vg_navigationFullHeight];
    CGFloat navTabBarY = 0.0f;
    CGFloat navTabBarW = SCREEN_WIDTH;
    CGFloat navTabBarH = NAV_TAB_BAR_HEIGHT;
    _navTabBar = [[SCNavTabBar alloc] initWithFrame:CGRectMake(navTabBarX, navTabBarY, navTabBarW, navTabBarH) showArrowButton:_showArrowButton];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    NSInteger tempIndex = _navTabBar.currentItemIndex;
    _navTabBar.currentItemIndex = _currentIndex;
    
    NSLog(@"%ld, %ld",tempIndex, _currentIndex);
    if (tempIndex != _currentIndex) {
        if (_delegate && [_delegate respondsToSelector:@selector(scrollViewDidScrollWithIndex:)]) {
            [_delegate scrollViewDidScrollWithIndex:_currentIndex];
        }        
    }
